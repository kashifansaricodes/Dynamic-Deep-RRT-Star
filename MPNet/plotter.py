...
# Jpg implementation
# '''
image_path = cv2.imread(f'C:/Users/sachi/Planning/Deep-RRT-Star-Implementation/images/117.jpg')

grid = cv2.cvtColor(image_path, cv2.COLOR_BGR2GRAY) 

start = np.array([89, 95])
goal = np.array([7,9])
numIterations = 700
stepSize = 5
        ...
        if rrtStar.goalFound(point):
            ...
            if projectedCost < rrtStar.goalCosts[-1]:
                ...
                for i in range(len(rrtStar.Waypoints)-1):
                    # plt.plot([rrtStar.Waypoints[i][0], rrtStar.Waypoints[j][0]], [rrtStar.Waypoints[i][1], rrtStar.Waypoints[j][1]],'ro', linestyle="--")
